      ...
      if (data is List) {
        final comentarios =
            (data).where((json) => json['noticiaId'] == noticiaId).map((json) {
              try {
                // Usar el método seguro para procesar todos los comentarios
                return Comentario.fromMapSafe(json);
              } catch (e) {
                debugPrint('❌ Error al deserializar comentario: $e');
                // Devolver un comentario placeholder en caso de error
                return Comentario(
                  noticiaId: noticiaId,
                  texto: 'Error al cargar comentario',
